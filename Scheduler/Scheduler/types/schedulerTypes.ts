...
export interface ISchedulerControlProps extends Partial<SchedulerProps<Event>> {
    ...
    /**
     * NEW — Callback when an existing event chip is moved (drag-drop) or
     * resized. resourceId is the resource the chip now belongs to.
     * Optional so existing callers (tests/mocks) don't need to supply it.
     */
