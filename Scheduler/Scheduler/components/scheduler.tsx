...
import { createEventClickedCallback, createNewEventCallback, createOnViewChangeCallback, createPrevClickCallback, createNextClickCallback, createSlotClickedFuncCallback, createToggleExpandFuncCallback, createMoveEventCallback, createUpdateEventStartCallback, createUpdateEventEndCallback } from "./callbacks";
...
const SchedulerControl: React.FC<ISchedulerControlProps> = React.memo((props) => {
    ...
    React.useEffect(() => {
        ...
        async function loadSchedulerData() {
            ...
            // NEW: enable drag-drop + resize. Set on the SPREAD (not the typed
            // literal above) so we never trip TS excess-property checks even if
            // a flag is absent from the SchedulerDataConfig type in a version.
            const schedulerConfig = {
                ...config,
                workWeekDays,
                movable: true,           // drag an event chip
                crossResourceMove: true, // allow dropping on a DIFFERENT resource row
                startResizable: true,    // drag left edge to change start (optional)
                endResizable: true,      // drag right edge to change end (optional)
            };
    ...
    // ----- NEW handlers: drag-move + resize -----
    // Handler: Event moved via drag-drop (callbacks/moveEvent.ts)
    const moveEvent = React.useCallback(
        createMoveEventCallback(dispatch, props.onMoveEvent),
        [dispatch, props.onMoveEvent]
    );

    // Handler: Event start resized (callbacks/updateEventStart.ts)
    const updateEventStart = React.useCallback(
        createUpdateEventStartCallback(dispatch, props.onMoveEvent),
        [dispatch, props.onMoveEvent]
    );

    // Handler: Event end resized (callbacks/updateEventEnd.ts)
    const updateEventEnd = React.useCallback(
        createUpdateEventEndCallback(dispatch, props.onMoveEvent),
        [dispatch, props.onMoveEvent]
    );
    // --------------------------------------------
